...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  useEffect(() => {
    ...
    /* Le délai laisse le temps au mouseenter de la carte voisine d'annuler la
       remise à zéro : sans lui, passer d'une carte à l'autre replierait
       l'éventail à chaque fois. */
    const scheduleReset = () => {
    ...
    /* Remise à zéro dès qu'on quitte une carte, et non le conteneur : celui-ci
       occupe toute la largeur de la section, il fallait donc éloigner la souris
       très loin avant que l'éventail se replie. */
    const leaveHandlers = visibleEntries.map(({ el }) => {
      el.addEventListener("mouseleave", scheduleReset);
      return { el, handler: scheduleReset };
    });

    const onMouseLeave = scheduleReset;
    ...
    return () => {
      ...
      leaveHandlers.forEach(({ el, handler }) =>
