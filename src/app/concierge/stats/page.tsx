  ...
  return (
    <div className="space-y-6">
      ...
      <div className="backdrop-blur-2xl bg-black/45 border border-white/[0.12] rounded-3xl p-6">
        <h1 className="text-2xl font-bold text-white font-[family-name:var(--font-manrope)]">
          Statistiques
        </h1>
        <p className="text-sm text-white/50 font-[family-name:var(--font-inter)] mt-1">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          >
            ...
            <p className="text-xs text-white/40 font-[family-name:var(--font-inter)] mt-1">
              {stat.label}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Chart */}
        <div className="backdrop-blur-2xl bg-black/45 border border-white/[0.12] rounded-3xl p-6">
          <h3 className="text-lg font-bold text-white font-[family-name:var(--font-manrope)] mb-6">
              ...
              {barChart.map((bar) => {
                ...
                return (
                  <div key={bar.label} className="flex-1 flex flex-col items-center gap-3">
                    ...
                    <span className={`text-xs font-medium ${isMax ? "text-white" : "text-white/40"} font-[family-name:var(--font-inter)]`}>
                      {bar.label}
                    </span>
                  </div>
                );
              ...
              {venueBreakdown.map((v) => (
                <div key={v.venue}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-bold text-white/70 font-[family-name:var(--font-manrope)]">
                      {v.commission}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-white/40 font-[family-name:var(--font-inter)] w-8 text-right">
                      {v.percentage}%
                    </span>
                  </div>
                </div>
        ...
        <div className="backdrop-blur-2xl bg-black/45 border border-white/[0.12] rounded-3xl p-6 lg:col-span-2">
          <h3 className="text-lg font-bold text-white font-[family-name:var(--font-manrope)] mb-4">
            Détail mensuel
          </h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.1]">
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white/40 font-[family-name:var(--font-inter)]">
                    Mois
                  </th>
                  <th className="text-center px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white/40 font-[family-name:var(--font-inter)]">
                    Couverts
                  </th>
                  <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white/40 font-[family-name:var(--font-inter)]">
                    Commission
                  </th>
                  <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-white/40 font-[family-name:var(--font-inter)]">
                    Croissance
                ...
                {monthlyData.length > 0 ? (
                  monthlyData.map((m) => (
                    <tr key={m.month} className="border-b border-white/[0.06] hover:bg-white/[0.03] transition-colors">
                      <td className="px-4 py-3.5 font-semibold text-white font-[family-name:var(--font-manrope)]">
                        {m.month}
                      </td>
                      <td className="px-4 py-3.5 text-center text-white/70 font-[family-name:var(--font-inter)]">
                        {m.covers}
                      </td>
                      <td className="px-4 py-3.5 text-right font-bold text-white font-[family-name:var(--font-manrope)]">
                        {m.commission} MAD
                      </td>
...
function EmptyState({ icon: Icon, text, sub }: { icon: typeof BarChart3; text: string; sub: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-sm font-medium text-white/40 font-[family-name:var(--font-inter)]">{text}</p>
      <p className="text-xs text-white/25 font-[family-name:var(--font-inter)] mt-1">{sub}</p>
    </div>
  );
}
